...
#'
#' @description
#' `hgch_scatter_NumNumNum()` Create a highcharter scatter plot based on a particular data type.
#' In this case, you can load data with only three columns,  this columns should are
#' **numeric class**, or be sure that the first three columns they meet this condition, the third column indicates the radius of the points
#'
#' @export
...
hgch_scatter_NumNumNum <- function(data, ...){
  ...
  l <- hgchmagic_prep(data, opts = opts, plot = "scatter", ftype = "Num-Num-Num")

  d <- l$d
  d$c <- scales::rescale(d$c, c(3, 15))
